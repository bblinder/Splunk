...
Automated regression coverage: [`tests/test_discovery.py`](../tests/test_discovery.py)
...
| `discovery_metadata.json` | | | Counts match on-disk files |
| `inventory_summary.md` | | | Generated Markdown catalog |
...
| `contact_methods_inventory.json` | Dict per username with `devices`, `emails`, `phones` | Devices only — multi-list response truncated |
| `paging_policies_inventory.json` | Dict per username with full policy objects | Partial policy data |
...
| `discovery_metadata.json` | Counts, `files_written`, `manual_capture_required` | Stale counts after partial re-run |
